...
export default function Checkbox({
  ...
  const baseId = useId();
  // selectionGeometries에서 해당 사이즈의 스타일 세트를 가져옴
  const styles = selectionGeometries[sizing];

  return (
    // 객체에 정의된 gap-2, gap-3 등을 적용
    <div className={twMerge("flex flex-col", styles.gap, className)}>
      {options.map((option) => {
